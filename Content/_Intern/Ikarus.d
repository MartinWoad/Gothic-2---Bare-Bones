...
func void MEMINT_OldWriteInt (var int address, var int val) {
    ...
    /* res wird nicht gebraucht, m�llt aber sonst den Stack zu! */
...
func void MEMINT_PrepareAssignments() {
    /* sorgt daf�r, dass MEMINT_Assign und MEMINT_StrAssign
    ...
    //Code �berschreiben. Vorsicht: Der erste Aufruf soll auch klappen!
    ...
    //alte Lesemethode muss aufr�umen
    MEM_Helper.enemy = 0;
...
func int MEM_ReadShort (var int adr) {
    return MEM_ReadInt (adr) & 65535;
};
...
func void MEM_AssignInst (var int inst, var int ptr) {
    ...
    if (ptr == 0 && !MEM_AssignInstSuppressNullWarning) {
		/* Instanzen die Null sind, will man eigentlich nicht, die machen nur �rger. */
...
func void MEM_AssignInstNull (var int inst) {
    /* Normalerweise will man Instanzen nicht zur�ck auf 0 setzen.
...
//--------------------------------------
// Unsinnig. Nur zur Abw�rtskompatibilit�t
// �berhaupt noch drin. Google sagt,
// Lehona hat es mal irgendwo benutzt.
//--------------------------------------

//L�sst currParserSymb auf das Symbol mit Instanz inst zeigen.
...
func int MEM_Alloc (var int amount) {
    ...
    /* string mit sich selbst konkatenieren bis gro� genug */
    var int size; size = 4;

    //VORSICHT! mindestens einmal muss die Schleife durchlaufen werden.
    //sonst kommt (vermutlich, nicht genau �berpr�ft) statisch die Adrese von der Parserkonstanten "AAAA" zur�ck!
...
func void MEM_Free (var int ptr) {
    ...
    /* Vorsicht: ptr in Strings zeigt auf das Byte nach dem ersten Reservierten!
     * Strings haben Referenzz�hler! Den Nullen! */
...
func void CALLINT_makecall (var int adr, var int cleanStack) {
    ...
    CALLINT_RetValIsFloat = false; //f�rs n�chste mal muss neugeschaltet werden.
    CALLINT_PutRetValTo   = 0;
...
func void MEM_ArrayRemoveIndex (var int zCArray_ptr, var int index) {
    ...
    //letzten Wert in die L�cke schieben
    array.numInArray -= 1;
...
func string STR_SubStr (var string str, var int start, var int count) {
    ...
    /* Hole Adressen von zwei Strings, Source und Destination (f�r Kopieroperation) */
    var zString zStrSrc;
...
/* urspr�nglicher Code von Gottfried */

/* STRINT_SplitArray enth�lt folgendes:
 *
 *    struct TStringInfo {
 *        int length;
 *        char* data;
...
//   unangetastet l�sst.
//
//   Zur Vermeidung tempor�rer kleiner
//   MEM_Alloc anfragen.
//--------------------------------------

/* Wei� nicht ob ich das mit h�tte reinnehmen sollen...
 * Aber warum nicht? */
 ...
 * ging n�mlich schief, weil STR_Compare oft string auf den Stack
 * schieben will! */
...
func int MEMINT_IsFrameBoundary(var int ESP) {
    const int retAdr = 0;
    if (!retAdr) {
        /* Wenn DoStack sich selbst aufruft, steht diese R�cksprungaddresse auf dem Stack: */
...
/*
    Leider werden manche Men�s jedesmal neu erzeugt (vom Script aus),
    andere dagegen werden beim ersten mal nach dem Spielstart erzeugt und dann behalten.
    Abh�ngig davon und von dem, was man eigentlich tun will, kann es n�tig sein
    in den Men�scripten �nderungen einzubringen (indem man
    in den Variablen dort schreibt) oder es ist n�tig sich das Men�
...
func int MEM_GetClassDef (var int objPtr) {
    ...
    //In obj._vtbl[0] steht die Adresse der Funktion, die ClassDef zur�ckgibt.
...
/* Danke an Gottfried f�r die Entdeckung von Wld_InsertObject! */
...
//--------------------------------------
//  Grundlage: Ist die Taste gedr�ckt?
//--------------------------------------

//etwas ungeschickt, dass die Methode, die auf KEY_HOLD pr�ft KeyPressed hei�t... :-(
//aber jetzt ist es so und ich wills nicht �ndern.
...
//  wann das erste mal gedr�ckt
//  und wann gehalten
//--------------------------------------

//Hier merke ich mir die Zust�nde seit der letzten Abfrage:
...
/* Problematisch, vielleicht gibt es irgendwann eine bessere L�sung.
...
func int MEMINT_OPT_FindEntry(var string optname) {
    //Anzahl Eintr�ge == 0 ausschlie�en (weil nur do-while schleife m�glich, keine while-do).
...
func int MEMINT_OPT_FindSection (var string sectname) {
    //Anzahl Sektionen == 0 ausschlie�en (weil nur do-while schleife m�glich, keine while-do).
...
func void MEMINT_PrintStackTrace_Implementation() {
    ...
    /* sehr ung�nstig: Im Stackframe der Funktion steht gar nicht die
     * aktuelle PopPos, die steht nur im Stackframe desjenigen obendr�ber
...
func void MEMINT_InitFasterReadWrite() {
     ...
     * kann so nicht funktionieren, schlie�lich wird MEM_WriteInt dazu gebraucht */
...
func void MEMINT_VersionError() {
    ...
    str = ConcatStrings(str, ", da sie Funktionalit�t aus dem Skriptpaket 'Ikarus' verwendet. Es ist wahrscheinlich, dass Gothic unmittelbar nach dieser Fehlermeldung abst�rzt. Die genannte Version von Gothic steht zum Beispiel auf worldofgothic.de zum Download bereit. Der merkw�rdige Charakter dieser Fehlermeldung ist leider nicht zu vermeiden. ### This mod only works with ");
    if (GOTHIC_BASE_VERSION == 1) {
        str = ConcatStrings(str, G1);
    } else {
