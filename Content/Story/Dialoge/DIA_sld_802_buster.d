...
func void dia_buster_aboutsentenza_info()
{
	...
	Info_AddChoice(dia_buster_aboutsentenza,"Trzymaj – 5 sztuk z³ota.",dia_buster_aboutsentenza_give);
};
...
func void dia_buster_bringtrophyshadowbeast_info()
{
	...
	else if(PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_SHADOWHORN] == FALSE)
	{
		...
		Info_AddChoice(dia_buster_bringtrophyshadowbeast,b_buildlearnstring("Naucz mnie.",b_getlearncosttalent(other,NPC_TALENT_TAKEANIMALTROPHY)),dia_buster_bringtrophyshadowbeast_teach);
	}
	else
	{
		bustertrophyshadowbeastcount = Npc_HasItems(other,itat_shadowhorn);
		xp_bringbustertrophyshadowbeast = 5 * XP_PER_VICTORY;
		bustersbustertrophyshadowbeastoffer = (VALUE_SHADOWHORN / 2) + BUSTER_BONUS;
		if(bustertrophyshadowbeastcount == 1)
		{
			...
			b_giveinvitems(other,self,4761,1);
		...
		else
		{
			...
			b_giveinvitems(other,self,4761,bustertrophyshadowbeastcount);
			Npc_RemoveInvItems(self,itat_shadowhorn,Npc_HasItems(self,itat_shadowhorn));
		...
		b_giveinvitems(self,other,5113,bustertrophyshadowbeastgeld);
	};
};
