...
func void DIA_Marduk_TEACH_Info()
{
	...
	if((Npc_GetTalentSkill(other,NPC_TALENT_MAGE) >= 3) && (PLAYER_TALENT_RUNES[SPL_IceCube] == FALSE) && (PLAYER_TALENT_RUNES[SPL_Icebolt] == TRUE))
	{
	...
	if((Npc_GetTalentSkill(other,NPC_TALENT_MAGE) >= 3) && (PLAYER_TALENT_RUNES[SPL_ChargeZap] == FALSE) && (PLAYER_TALENT_RUNES[SPL_Zap] == TRUE))
	{
	...
	if((Npc_GetTalentSkill(other,NPC_TALENT_MAGE) >= 4) && (PLAYER_TALENT_RUNES[SPL_LightningFlash] == FALSE) && (PLAYER_TALENT_RUNES[SPL_Zap] == TRUE))
	{
	...
	if((Npc_GetTalentSkill(other,NPC_TALENT_MAGE) >= 5) && (PLAYER_TALENT_RUNES[SPL_IceWave] == FALSE) && (PLAYER_TALENT_RUNES[SPL_IceCube] == TRUE))
	{
...
func void DIA_Marduk_Kap3_Hello_Info()
{
	...
	if(hero.guild == GIL_PAL)
	{
		Info_AddChoice(DIA_Marduk_Kap3_Hello,"Tylko na krótki czas.",DIA_Marduk_Kap3_Hello_Soon);
	};
	if(hero.guild == GIL_DJG)
	{
...
func void DIA_Marduk_Kap3_Hello_NotYourConcern()
{
	...
	if(hero.guild == GIL_PAL)
	{
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_01");	//(z nagan¹) Paladyn powinien byæ zawsze uprzejmy i skromny. Jego zadaniem jest chroniæ tych, którzy nie potrafi¹ obroniæ siê sami.
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_02");	//(z nagan¹) Powinieneœ byæ wdziêczny Innosowi, ¿e powierzy³ ci tak zaszczytn¹ misjê. Przemyœl to sobie!
	};
	if(hero.guild == GIL_DJG)
	{
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_03");	//(ze z³oœci¹) W dawnych czasach ho³ota nie mia³a wstêpu do naszego œwiêtego klasztoru. Jesteœ ¿ywym dowodem na to, ¿e by³y to lepsze czasy.
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_04");	//(ostrzegawczym tonem) Ostrzegam, pilnuj siê, jeœli nie chcesz zostaæ ukarany. Taka arogancja nie jest tu mile widziana..
	};
	Info_ClearChoices(DIA_Marduk_Kap3_Hello);
};
...
{
	AI_Output(other,self,"DIA_Marduk_Kap3_Hello_Soon_15_00");	//Tylko na krótki czas.
...
func void DIA_Marduk_Kap3_PERM_Info()
{
	...
	AI_Output(self,other,"DIA_Marduk_Kap3_PERM_05_04");	//(zmartwiony) Wróg najwyraŸniej wkroczy³ ju¿ do miasta.
	...
	AI_Output(self,other,"DIA_Marduk_Kap3_PERM_05_07");	//(z gniewem) W bia³y dzieñ! To zasz³o za daleko. Niestety, obawiam siê, ¿e to mo¿e byæ dopiero pocz¹tek.
