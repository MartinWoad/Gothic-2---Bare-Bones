...
func void dia_thorben_zustimmung_info()
{
	...
	if(VATRAS_SEGEN == TRUE)
	{
		...
		if((DARON_SEGEN == TRUE) || (ISGAROTH_SEGEN == TRUE) || (other.guild == GIL_KDF))
		{
			...
			Log_CreateTopic(TOPIC_LEHRLING,LOG_MISSION);
			Log_SetTopicStatus(TOPIC_LEHRLING,LOG_RUNNING);
			b_logentry(TOPIC_LEHRLING,"Thorben przyjmie mnie na czeladnika.");
		}
		else
		{
...
instance DIA_THORBEN_TEACH(C_INFO)
{
	...
	permanent = TRUE;
	description = b_buildlearnstring("Naucz mnie otwieraæ zamki!",b_getlearncosttalent(other,NPC_TALENT_PICKLOCK));
};
